...
Defines the canonical domain taxonomies for Decision Mirror: a fixed decision-category
enum and a fixed catalog of eight cognitive biases, each backed by a single Zod-schema
source of truth that all producers and consumers reuse rather than redeclare.
