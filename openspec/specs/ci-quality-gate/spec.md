...
Defines the continuous-integration quality gate for Decision Mirror: GitHub Actions workflows
that run the deterministic repository gate on pull requests and pushes to `main`, backed by a
real Postgres-with-pgvector service for database tests, with no real AI provider calls, while
production deployment remains owned by Vercel's Git integration.
