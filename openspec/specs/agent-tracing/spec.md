# agent-tracing Specification

## Purpose

Define LangSmith tracing of the `runAgent` graph so each invocation is recorded as a
per-node run tree with token and recalled-memory metadata, scoped to a configured project
with retention, and inert when unconfigured — to diagnose weak output and irrelevant memory
recall without exposing more decision content than the trace inputs already require.
