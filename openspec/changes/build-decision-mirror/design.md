...
### D5 — Agentic pipeline: LangGraph.js (in-process) wrapping Anthropic Claude with enforced structured output
The analysis runs as a compiled LangGraph.js `StateGraph` invoked in-process by `runAgent(decisionId)` — no separate service or second runtime. The graph nodes are `load-memory → analyze → validate → persist+remember`, with a `fail` branch. The `analyze` node calls Claude through a single provider wrapper; the model returns analysis via a tool/JSON schema so output is machine-validated; results are parsed against a strict Zod schema before persistence, and a schema-validation failure routes to `fail` (status `failed`, eligible for retry). Default model favors latency/cost for a demo and is configurable via env. Prompt caching is used for the static system/instruction portion. The graph buys composable nodes, retry/resume via a checkpointer (see D10), and per-run tracing via LangSmith (see D13).
- *Alternatives considered:* a bare Anthropic SDK call inline in `after()` (the original plan — simplest, but no node composition, no checkpointer resume, weaker tracing story); a **separate Python LangGraph service** (full ecosystem + native memory primitives, but two deploys and two languages — rejected to keep one TypeScript deployment); LangGraph Platform managed cloud (built-in persistence/memory but vendor lock + cost); free-text completion then regex/parse (brittle); OpenAI (equivalent capability; Claude chosen for structured-output ergonomics and project default).
...
- **In-process serverless execution time** → the full graph (memory recall + Claude call + embedding write) must fit within the function limit; the polling + retry/stalled-timeout design means an interrupted run is recoverable, and the scale-up seam (D4) is the documented path if runs grow.
