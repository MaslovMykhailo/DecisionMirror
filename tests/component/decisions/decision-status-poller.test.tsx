...
function statusResponse(status: "processing" | "ready" | "failed", failureReason?: string) {
  return new Response(
    JSON.stringify({
...
describe("decision status poller", () => {
  ...
  it("stops on failed without refreshing the route", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(statusResponse("processing"))
      .mockResolvedValueOnce(statusResponse("failed", "The structured output did not match."));
    const onStatusChange = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
